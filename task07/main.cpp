  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = (1.f / nsample) * (hit1_rad * hit0_brdf * hit0_refl.dot(hit0_normal)) / hit0_pdf; // replace this with some code
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float rad = (1.f / num_half_sample) * (w_brdf * hit1_rad * hit0_brdf * hit0_refl.dot(hit0_normal)) / hit0_pdf_brdf_sample; // write some code
      ...
      for (int isample = 0; isample < nsample / 2; ++isample) {
        ...
        float rad = (1.f / num_half_sample) * (w_light * hit1_rad * hit0_brdf * hit0_refl.dot(hit0_normal)) / hit0_pdf_light_sample; // write some code
        img_mis[ih * img_width + iw] += rad;
      }
    }
  }
